...
| `test.max_configs_to_test` | سقف کانفیگ تست‌شده در هر سیکل. **زمان سیکل یعنی تازگی لیست** — این نودها در چند دقیقه می‌میرند، پس لیستی که هر ۲۰ دقیقه ساخته شود تا وقتی اپ بخواندش نیمی‌اش غلط است. اگر استخر از این عدد بزرگ‌تر باشد، هر سیکل **همهٔ نودهای دارای سابقه** (یعنی همان لیست منتشرشده) را تست می‌کند به‌علاوهٔ یک **پنجرهٔ چرخشی** روی بقیه؛ پس جاروی کامل استخر با همان سرعت قبل انجام می‌شود ولی خود لیست هر سیکل بازبینی می‌شود. `0` = همه‌چیز |
